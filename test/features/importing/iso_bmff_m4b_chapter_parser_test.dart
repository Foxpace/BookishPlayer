...
void main() {
  test(
    'parses embedded chpl chapters without reading media payloads',
    () async {
  ...
  test('ignores non-M4B files', () async {
    expect(
  ...
  test('uses the custom ID3 CHAP fallback for MP3 chapters', () async {
    final temporary = await Directory.systemTemp.createTemp(
  ...
  test('parses chapters from the real M4B sample', () async {
    final sample = File(
    ...
    expect(chapters.first.title, '00 - Introductory Memoir by Edmund W. Gosse');
    expect(chapters.first.startMs, 0);
