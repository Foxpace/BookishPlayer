...
EmbeddedTextMetadata readMp4TextMetadata(
  RandomAccessFile file,
  int fileLength,
) {
  final tags = <String, String>{};
  _scanMp4Text(file, 0, fileLength, 0, tags);
  return embeddedTextMetadataFromTags(tags);
}
...
String? _readMp4ItemText(RandomAccessFile file, int start, int end) {
  ...
  while (offset + 8 <= end) {
    ...
    if (box.type == 'data' && box.end - box.payloadStart > 8) {
      return _readMp4TextValue(file, box, prefixLength: 8);
    }
...
MapEntry<String, String>? _readMp4FreeformText(
  ...
  while (offset + 8 <= end) {
    ...
    if (box.type == 'name' && box.end - box.payloadStart > 4) {
      name = _readMp4TextValue(file, box, prefixLength: 4);
    } else if (box.type == 'data' && box.end - box.payloadStart > 8) {
      value = _readMp4TextValue(file, box, prefixLength: 8);
    }
...
String? _readMp4TextValue(
  RandomAccessFile file,
  EmbeddedMetadataBox box, {
  required int prefixLength,
}) {
  final valueStart = box.payloadStart + prefixLength;
  final valueLength = box.end - valueStart;
  final bytes = readMetadataBytes(file, valueStart, valueLength);

  return cleanEmbeddedText(utf8.decode(bytes, allowMalformed: true));
}

EmbeddedTextMetadata readFlacTextMetadata(
  RandomAccessFile file,
  int fileLength,
) {
  var offset = 4;
  while (offset + 4 <= fileLength) {
    ...
    if (type == 4) {
      return embeddedTextMetadataFromTags(
        _decodeVorbisComments(readMetadataBytes(file, offset, length)),
      );
    }
...
EmbeddedTextMetadata readWaveTextMetadata(
  RandomAccessFile file,
  int fileLength,
) {
  var offset = 12;
  while (offset + 8 <= fileLength) {
    ...
    if (type.toLowerCase() == 'id3 ') {
      return readId3Metadata(file, payloadStart)?.text ??
...
EmbeddedTextMetadata readOggTextMetadata(
  RandomAccessFile file,
  int fileLength,
) {
  var offset = 0;
  ...
  while (offset + 27 <= fileLength && offset < 64 * 1024 * 1024) {
    ...
    for (final segmentLength in lacing) {
      ...
      if (segmentLength < 255) {
        ...
        if (comments != null) {
          return embeddedTextMetadataFromTags(comments);
        }
      }
    }
...
Map<String, String> _decodeVorbisComments(Uint8List bytes, {int offset = 0}) {
  ...
  final vendorLength = readUint32LittleEndian(bytes, cursor);
  ...
  final count = readUint32LittleEndian(bytes, cursor);
  cursor += 4;

  for (var index = 0; index < count && cursor + 4 <= bytes.length; index++) {
    final length = readUint32LittleEndian(bytes, cursor);
    cursor += 4;
    ...
    final value = cleanEmbeddedText(comment.substring(separator + 1));
    if (value != null) {
