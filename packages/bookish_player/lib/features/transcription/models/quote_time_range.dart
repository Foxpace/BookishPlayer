...
class QuoteTimeRange {
  ...
  QuoteTimeRange withPreset(Duration length) {
    ...
    return _copyRange(startMs: math.max(0, endMs - requested), endMs: endMs);
  }

  QuoteTimeRange withStart(Duration value) => _copyRange(
    startMs: value.inMilliseconds.clamp(0, endMs - minimumLengthMs),
    endMs: endMs,
  );

  QuoteTimeRange withEnd(Duration value) => _copyRange(
    startMs: startMs,
  ...
  QuoteTimeRange shift(Duration delta) {
    ...
    return _copyRange(startMs: shiftedStart, endMs: shiftedEnd);
  }

  QuoteTimeRange _copyRange({required int startMs, required int endMs}) {
    assert(startMs >= 0);
