...
void main() {
  group('Quote time range', () {
    group('QuoteTimeRange', () {
      test('Given the quote time range, When its behavior is exercised, Then starts with the last 30 seconds before the chapter anchor', () {
        // WHEN
      ...
      test('Given the quote time range, When its behavior is exercised, Then presets change only the start and keep the endpoint stable', () {
        // WHEN
      ...
      test('Given the quote time range, When its behavior is exercised, Then preset clamps at the beginning without moving the endpoint', () {
        // GIVEN
        final initial = QuoteTimeRange.initial(
      ...
      test('Given the quote time range, When its behavior is exercised, Then start and end controls cannot cross or leave the chapter', () {
        // GIVEN
        final initial = QuoteTimeRange.initial(
      ...
      test('Given the quote time range, When its behavior is exercised, Then manual boundaries preserve exact millisecond precision', () {
        // GIVEN
        final initial = QuoteTimeRange.initial(
          chapterDuration: const Duration(minutes: 10),
          anchor: const Duration(minutes: 5),
      ...
      test('Given the quote time range, When its behavior is exercised, Then shifting preserves duration and clamps at both boundaries', () {
        // GIVEN
        final initial = QuoteTimeRange.initial(
      ...
      test('Given the quote time range, When its behavior is exercised, Then very short chapters still produce a valid full range', () {
        // WHEN
        final range = QuoteTimeRange.initial(
