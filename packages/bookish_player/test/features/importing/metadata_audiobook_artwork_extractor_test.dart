...
void main() {
  group('Metadata audiobook artwork extractor', () {
    test('Given the metadata audiobook artwork extractor, When its behavior is exercised, Then extracts embedded MP4/M4B cover artwork to durable storage', () async {
      // GIVEN
      ...
      try {
        ...
        expect(_readArtwork(artworkPath), imageBytes);
      } finally {
        await temporary.delete(recursive: true);
      }
    });

    test('Given the metadata audiobook artwork extractor, When its behavior is exercised, Then extracts ID3 APIC cover artwork without a metadata plugin', () async {
      // GIVEN
      final temporary = await Directory.systemTemp.createTemp(
    ...
    test('Given the metadata audiobook artwork extractor, When its behavior is exercised, Then extracts Opus comment cover artwork without a metadata plugin', () async {
      // GIVEN
      final temporary = await Directory.systemTemp.createTemp(
...
List<int> _readArtwork(String? path) {
  if (path == null) {
    fail('Artwork extraction must return a path.');
  }
