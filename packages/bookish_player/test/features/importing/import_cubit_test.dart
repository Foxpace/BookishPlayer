...
void main() {
  group('Import cubit', () {
    ...
    test('Given the import cubit, When its behavior is exercised, Then cancelled Android picker remains visible and can be retried', () async {
      // GIVEN
    ...
    test('Given the import cubit, When its behavior is exercised, Then normal import clears picker cache after the copied book is saved', () async {
      // GIVEN
      final events = <String>[];
      final files = FakeImportFiles(const [
        SelectedAudioFile(
    ...
    test('Given the import cubit, When its behavior is exercised, Then unrelated selected files are imported as separate books', () async {
      // GIVEN
    ...
    test('Given the import cubit, When its behavior is exercised, Then numbered files are also imported as separate books', () async {
      // GIVEN
      final books = FakeImportBooks(<String>[]);
      final files = FakeImportFiles(const [
        SelectedAudioFile(
    ...
    test('Given the import cubit, When its behavior is exercised, Then embedded metadata is applied conservatively during import', () async {
      // GIVEN
      final books = FakeImportBooks(<String>[]);
      final files = FakeImportFiles(const [
        SelectedAudioFile(
    ...
    test('Given one completed book and another copying, When the user cancels, Then the cubit preserves partial success in its route result', () async {
      // GIVEN
      final books = FakeImportBooks(<String>[]);
      final files = FakeImportFiles(const [
        SelectedAudioFile(
