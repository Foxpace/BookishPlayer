...
void main() {
  group('Audiobook dao note archive', () {
    ...
    test('Given the audiobook dao note archive, When its behavior is exercised, Then book deletion preserves normalized notes and reconnects on import', () async {
      // GIVEN
      ...
      expect((await sut.getBookMetadata()).single.activeBookId, reimported.id);
      expect((await sut.getNotes(reimported.id)).single.id, 'note-id');
    });

    test('Given the audiobook dao note archive, When its behavior is exercised, Then listening history survives deletion and accumulates after reimport', () async {
      // GIVEN
      final original = Audiobook(
        id: 'history-book',
    ...
    test('Given the audiobook dao note archive, When its behavior is exercised, Then delete everything removes notes, metadata, and history', () async {
      // GIVEN
    ...
    test('Given the audiobook dao note archive, When its behavior is exercised, Then metadata parse failures identify the stored audiobook record', () async {
      // GIVEN
