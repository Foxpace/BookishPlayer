...
void main() {
  ...
  group('Supported and internally consistent backup', () {
    test('Given a supported and internally consistent backup, When the payload is validated, Then validation completes without a failure', () {
      // THEN
  ...
  group('Backup with duplicate identifiers', () {
    test('Given a backup with duplicate identifiers, When the payload is validated, Then a typed duplicate failure is reported', () {
      // WHEN
  ...
  group('Note that references missing book metadata', () {
    test('Given a note that references missing book metadata, When the payload is validated, Then restoration is rejected before persistence is changed', () {
      // WHEN
      final backup = backupFixture(
        content: (
          books: null,
  ...
  group('Backup from a newer incompatible schema', () {
    test('Given a backup from a newer incompatible schema, When the payload is validated, Then an unsupported-version failure is reported', () {
      // THEN
      expect(
