...
void main() {
  group('Bookish theme', () {
    test('Given the bookish theme, When its behavior is exercised, Then provides distinct complete light and dark themes', () {
      // THEN
      ...
      expect(BookishTheme.light.textTheme.diagnostics?.fontFamily, 'monospace');
      expect(
        BookishTheme.light.dropdownMenuTheme.textStyle?.fontFamily,
        'Roboto',
      );
      expect(BookishTheme.light.popupMenuTheme.textStyle?.fontFamily, 'Roboto');
      expect(
    ...
    test('Given manual app colors, When different primary colors are selected, Then each theme derives a distinct Material color scheme', () {
      // WHEN
    ...
    test('Given the app theme changes, When the status bar renders, Then its text brightness follows the theme', () {
      // GIVEN
    ...
    test('Given an Android system color scheme, When the theme is built, Then system surfaces and primary colors are used', () {
      // GIVEN
