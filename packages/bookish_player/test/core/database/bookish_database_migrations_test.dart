...
void main() {
  ...
  group('Version-one database with an invalid stored theme', () {
    test('Given a version-one database with an invalid stored theme, When the database is opened by the current application, Then it is migrated transactionally to a safe theme', () async {
      ...
      final legacy = await factory.openDatabase(
        ...
        version: 1,
        onVersionChanged: (database, _, _) => settings.record('appearance').put(
          database,
          {'theme': 'removed-theme', 'retained': true},
        ),
      );
      await legacy.close();
  ...
  group('Existing supported appearance preference', () {
    test('Given an existing supported appearance preference, When the database schema is upgraded, Then the preference is preserved', () async {
      // GIVEN
      final factory = newDatabaseFactoryMemory();
      final legacy = await factory.openDatabase(
