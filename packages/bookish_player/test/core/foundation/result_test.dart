...
void main() {
  group('Application result', () {
    test('Given a caught exception, When an operation failure is created, Then its category, detail, and error remain available', () {
      // GIVEN
      final error = Exception('disk unavailable');

      // WHEN
      final failure = AppFailure.operationFailed('library.load', error: error);
    ...
    test('Given work completed before cancellation, When a failed result is returned, Then partial success data remains available', () {
      // WHEN
