...
class SpeechModelCatalogCache {
  ...
  List<CactusSpeechModel> _readCatalog() {
  ...
  void write(List<CactusSpeechModel> models) {
...
List<CactusSpeechModel> reconcileSpeechModels({
  required List<CactusSpeechModel> catalog,
  required Set<String> downloadedSlugs,
}) {
  final models = <CactusSpeechModel>[];
  final seen = <String>{};
  ...
  final localOnly =
      downloadedSlugs.where((slug) => seen.contains(slug) == false).toList()
        ..sort();

  models.addAll(
    localOnly.map((slug) => CactusSpeechModel(slug: slug, isDownloaded: true)),
  );
